<template>
	<view class='up_box'>
		...
		<button v-if='data.isbind==0' @click='bindMobile'>绑定</button>
		<button v-else @click='changeMobile'>更换</button>
	</view>
</template>
...
<script setup>
	...
	// 验证码是否正确
	const changeMobile = () => {
		proxy.$api.user.changeMobile(data.formData).then(res=>{
			if(res.code==200){
				uni.navigateTo({
					url: '/pages/update/changeMobile'
				})
			}else{
				uni.showToast({
					title: res.message
